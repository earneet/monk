...
## 执行中变更(用户反馈重构,2026-07-12)

用户验证后反馈:全图哈密顿生成「意义何在」——monk 趣味在机制+障碍多样性,全填满空解需手动删/标,价值有限。重构:
- **加 `generate_random_walk(size, start, end)`**:**部分路径**(随机游走,不覆盖全图),用户在路径外填障碍构成关卡。设终点时 DFS 保证到达(`_dfs_to_end`);不设终点随机游走到死路。
- **算法下拉加「随机游走」**:第 4 个选项(推荐用法)。
- **起终点输入**:起点/终点 LineEdit(`x,y` 格式,终点可选),生成前可设。
- **canvas `_draw` 加路径连线 + 起点圆**:可视化路径走向,区分算法形状(螺旋/Hilbert/启发式/随机游走)。
- **保留** spiral/hilbert/heuristic(全图生成,可选)。
- §1-§3「覆盖全网格」定位以 `generate_random_walk`(部分路径)为推荐用法为准;全图算法作辅助。
